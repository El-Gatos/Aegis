    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const snapshot = await logsRef
                ...
                .where('action', '==', 'warn') // Filter specifically for 'warn' actions
            ...
            const warningsEmbed = new EmbedBuilder()
                .setColor(0xFFCC00) // A yellow color for warnings
                .setAuthor({ name: `Warning History for ${target.tag}`, iconURL: target.displayAvatarURL() })
                .setTimestamp();
            ...
            snapshot.forEach(doc => {
                ...
                const discordTimestamp = `<t:${Math.floor(timestamp.getTime() / 1000)}:f>`; // Full date/time format
            ...
            await interaction.editReply({ content: "An error occurred while fetching the user's warnings." });
        }
    }
};
