...
export const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const deleteMessageDays = interaction.options.getInteger(
      "delete_messages",
      true
    );
    const reason =
    ...
    if (
      ...
      await interaction.reply({
        content:
          "You can't softban a member with an equal or higher role than you.",
        flags: [MessageFlags.Ephemeral],
      });
      return;
    }
    ...
    if (!target.bannable) {
      await interaction.reply({
        content:
          "I don't have permission to perform this action on that member.",
        flags: [MessageFlags.Ephemeral],
      });
      return;
    }
    ...
    try {
      ...
      if (interaction.deferred) {
        await interaction.editReply({
          content:
            "An unexpected error occurred while trying to softban the member.",
        });
      } else {
        await interaction.reply({
          content:
