...
export const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const kickEmbed = new EmbedBuilder()
      ...
      .setColor("Red")
      .setDescription(
        `User: **${target.user.tag}** has been kicked from **${interaction.guild.name}** for the following reason:\n\n${reason}`
      )
      .setTimestamp();
    ...
    try {
      ...
      const errorMessage = {
        content:
          "An unexpected error occurred. The user may have been kicked, but the action could not be logged.",
      };
