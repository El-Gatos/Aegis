...
export const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!target.moderatable) {
      await interaction.reply({
        content:
          "I don't have permission to unmute that member. They may have a higher role than me.",
        flags: [MessageFlags.Ephemeral],
      });
      return;
    }
    ...
    try {
      ...
      await interaction.reply({
        content: `Successfully unmuted **${target.user.tag}**. Reason: ${reason}`,
        flags: [MessageFlags.Ephemeral],
      });
      ...
      await interaction.reply({
        content:
          "An unexpected error occurred while trying to unmute the member.",
        flags: [MessageFlags.Ephemeral],
      });
    }
  },
