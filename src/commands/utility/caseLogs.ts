...
import { SlashCommandBuilder, PermissionFlagsBits, ChatInputCommandInteraction, MessageFlags, EmbedBuilder } from 'discord.js';
...
export const command: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            if (totalLogs === 0) {
                await interaction.editReply({ content: 'No moderation history found for this server.' });
                return;
            }
            const totalPages = Math.ceil(totalLogs / logsPerPage);

            if (page > totalPages) {
                await interaction.editReply({ content: `Invalid page. This server only has ${totalPages} page(s) of logs.` });
                return;
            }

            // Get the paginated logs
            const snapshot = await logsRef.orderBy('timestamp', 'desc').limit(logsPerPage).offset(offset).get();

            const historyEmbed = new EmbedBuilder()
                .setColor(0x5865F2)
                .setAuthor({ name: `Server Moderation History - Page ${page}/${totalPages}`, iconURL: interaction.guild?.iconURL() || undefined })
                .setTimestamp()
            ...
            await interaction.editReply({ content: "An error occurred while fetching the server's history." });
        }
    }
};
